...
class DiscordMusicBot extends Client {
  constructor(props) {
    ...
    this.Manager = new Manager({
      ...
      .on("nodeError", (node, error) =>
        this.log(
          `Lavalink: Node ${node.options.identifier} bir hata vardı: ${error.message}`
        )
      )
      .on("trackStart", async (player, track) => {
        this.SongsPlayed++;
        let TrackStartedEmbed = new MessageEmbed()
          .setAuthor(`Şuan Çalıyor ♪`, this.botconfig.IconURL)
          .setThumbnail(player.queue.current.displayThumbnail())
          .setDescription(`[${track.title}](${track.uri})`)
          .addField("Kişi Tarafından Açıldı", `${track.requester}`, true)
          .addField(
            "SÜRE",
  ...
  sendError(Channel, Error) {
    let embed = new MessageEmbed()
      ...
      .setFooter(
        "Bunun bir hata olduğunu düşünüyorsanız, lütfen bunu destek sunucusuna bildirin.!"
      );
